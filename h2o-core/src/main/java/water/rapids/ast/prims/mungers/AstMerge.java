...
/*        if (lv.isNumeric() && !lv.isInt())
    ...
    method = "radix"; // method set to radix only as hash method does not work if rite frame contains duplicate key values
...
/*      if (allRite)
      ...
      if (onlyLeftAllOff) {
        ...
        mergedFrame.reOrder(colMapping);  // reorder the frame columns
        return tempFrame;
